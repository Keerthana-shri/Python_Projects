...
def update_pokemon(pokemon_id: int, updated_pokemon: Pokemon):
    for index, pokemon in enumerate(pokemon_data):
        if pokemon["id"] == pokemon_id:
            ...
            updated_data["stats"] = [
                stat.model_dump() for stat in updated_pokemon.stats
            ]
            updated_data["types"] = [
            ...
            return {
                "message": f"Pokemon {updated_pokemon.name} was updated successfully"
            }
    raise HTTPException(status_code=404, detail="Pokemon not found")
...
def delete_pokemon(pokemon_id: int):
    for index, pokemon in enumerate(pokemon_data):
        if pokemon["id"] == pokemon_id:
            ...
            return {
                "message": f"Pokemon {deleted_pokemon['name']} was deleted successfully"
            }
    raise HTTPException(status_code=404, detail="Pokemon not found")
